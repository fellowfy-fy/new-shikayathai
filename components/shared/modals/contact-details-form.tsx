...
export const ContactDetailsForm: React.FC<ContactDetailsFormProps> = ({
  ...
  const { showModal, hideModal } = useModalStore(); // Используем Zustand для управления модалом
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    //   complaintData.photos.forEach((photo: File) => formData.append("photos", photo));
    //   complaintData.documents.forEach((document: File) =>
    ...
    //     dataLayer.push({ event: "share_modal_showed" });
    //   }

    //   // Показываем компонент FacebookShare через Zustand
      ...
      <div className="relative bg-white p-6 rounded-[32px] shadow-lg w-full max-w-[800px] h-auto overflow-hidden">
        <div className="w-[257px] h-4 flex justify-between items-center z-10 ">
          ...
          <button
            ...
            onClick={hideModal} // Закрытие через Zustand
          >
